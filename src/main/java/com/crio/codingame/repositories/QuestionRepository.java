...
public class QuestionRepository implements IQuestionRepository {
    ...
    public List<Question> findAll() {
        List<Question> values = new ArrayList<>(questionMap.values());
        return values;
    }
    ...
    public List<Question> findAllQuestionLevelWise(Level level) {
        List<Question> qlevel = questionMap.values().stream().filter(x->x.getLevel()==level).collect(Collectors.toList());
     return qlevel;
    }
